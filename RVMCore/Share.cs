...
namespace RVMCore
{
    public static class Share
    {
         /// <summary>
            ...
            /// </summary>
            /// <returns>a string looks like "[Q1'18,Q1'18]"</returns>
            public static string GetTimeSpan(ProgramTimeSpan span)
            {
            ...
            {
                str = CheckString(str); // Make it more eazy to process.
                var SharpIndex = str.IndexOf("#");
                if (SharpIndex > 0)
                    return CheckAgain(Strings.Trim(str.Substring(0, SharpIndex)));// Convert to full-width char to prevent IO errors.
                                                                      // Find pattern'<第>[Number]<话，回>
                string pet = @"[\\p{N}第\(（][\\p{N}零一二三四五六七八九十百千萬億万亿壹貳叄肆伍陸柒捌玖拾佰仟贰叁陆\d]+[\\p{N}\)）話回]";
                if (Regex.IsMatch(str, pet))
                {
                    var match = Regex.Match(str, pet);
                    SharpIndex = str.IndexOf(match.Value);
                    if (SharpIndex > 0)
                        return CheckAgain(Strings.Trim(str.Substring(0, SharpIndex)));
                }
            ...
            // If Regex.IsMatch(str, pet) Then
            // Dim match = Regex.Match(str, pet)
            // SharpIndex = str.IndexOf(match.Value)
            // If SharpIndex > 0 Then
            // Return StrConv(Trim(str.Substring(0, SharpIndex)), VbStrConv.Wide)
            // End If
            // End If
            return CheckAgain(str.Trim());
            }
            private static string CheckString(string input)
            {
                string str = input.Replace("＃", "#")
                                  ...
                                  .Replace(">", "＞")
                                  .Replace("/", "／")
                                  .Replace(@"\", Strings.StrConv(@"\", VbStrConv.Wide))
                                  .Replace(":", Strings.StrConv(":", VbStrConv.Wide))
                                  .Replace("\"", Strings.StrConv("\"", VbStrConv.Wide))
                                  .Replace("|", Strings.StrConv("|", VbStrConv.Wide));
                return Regex.Replace(str, @"(?<=[aA-zZ0-9])\s(?=[aA-zZ0-9])", " ");
            }
        public static string CheckAgain(string tile)
        {
            string str = tile;
            ...
            if (str.EndsWith("△")) str = str.Substring(0, str.Length - 1);
            if (str.EndsWith("●")) str = str.Substring(0, str.Length - 1);
            if (str.EndsWith("〇")) str = str.Substring(0, str.Length - 1);
            if (str.EndsWith("◎")) str = str.Substring(0, str.Length - 1);
            if (str.StartsWith("["))
            {
                string pet = @"^(\[+[\u4e00-\u9fa5]+\])+";
            ...
            public static void RenameDirUpToDate(ref string dirPath, DateTime newDate)
            {
                ...
                if (DateTime.Compare(OldDate.EndQuarterly, newDate) < 0)
                {
                    ...
                    dirName = System.IO.Path.Combine(System.IO.Path.GetDirectoryName(dirPath), GetTimeSpan(OldDate) + dirName.Substring(i2 + 1));
                    try
                    {
